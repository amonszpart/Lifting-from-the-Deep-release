...
def import_json(path='json/MPI_annotations.json', order='json/MPI_order.npy'):
    ...
    is_not_validation = [not data_this[i]['isValidation'] for i in range(num_samples)]
...
def generate_labels(image_shape, joint_positions, num_other_people, joints_other_people, offset):
    """Given as input a set of joint positions and the size of the input image it generates
    a set of heat-maps of the same size. It generates both heat-maps used as labels for the
    first stage (label_1st_lower) and for all the other stages (label_lower)."""
    _FILTER_JOINTS = np.array([9, 8, 12, 11, 10, 13, 14, 15, 2, 1, 0, 3, 4, 5])

    img_height, img_width, _ = image_shape
    heat_maps_single_p = np.zeros((config.NUM_OUTPUT, config.INPUT_SIZE, config.INPUT_SIZE))
    heat_maps_other_p = np.zeros((config.NUM_OUTPUT, config.INPUT_SIZE, config.INPUT_SIZE))
    ...
    for i in range(config.NUM_JOINTS):
        ...
        if not skip:
            heat_maps_single_p[i] = gaussian_heatmap(config.INPUT_SIZE, config.INPUT_SIZE,
                                                     curr_joint[1] - offset[1], curr_joint[0] - offset[0],
                                                     config.SIGMA, config.SIGMA)
            heat_maps_other_p[i] = gaussian_heatmap(config.INPUT_SIZE, config.INPUT_SIZE,
                                                    curr_joint[1] - offset[1], curr_joint[0] - offset[0],
                                                    config.SIGMA, config.SIGMA)
    heat_maps_single_p[-1] = np.maximum(1 - np.max(heat_maps_single_p[:-1], axis=0),
    ...
    heat_maps_other_p[-1] = np.maximum(1 - np.max(heat_maps_other_p[:-1], axis=0),
                                       np.zeros((config.INPUT_SIZE, config.INPUT_SIZE)))
...
def generate_center_map(center_pos, img_shape):
    """Given the position of the person and the size of the input image it generates
    a heat-map where a gaissian distribution is fit in the position of the person in the image."""
    img_height = img_shape
    img_width = img_shape
    center_map = gaussian_heatmap(img_height, img_width, center_pos[1], center_pos[0],
...
def rescale(data, new_size):
    ...
    resized_data = cv2.resize(data, (new_size, new_size), interpolation=cv2.INTER_CUBIC)
    return resized_data


def crop_image(image, obj_pose):
    """Crop the image in order to have the person at the center and the final image size
    is the same as the expected CNN input size.
    Returns the cropped image and the offset that is used to update the joint positions."""
    ...
    padded_image = np.lib.pad(image, ((pad_up, pad_bottom), (pad_left, pad_right), (0, 0)),
                              'constant', constant_values=((0, 0), (0, 0), (0, 0)))

    cropped_image = padded_image[offset_up + pad_up: offset_up + pad_up + config.INPUT_SIZE,
